...
- `npm run size` — gzipped JS/CSS sizes. **As of 1.23.0: ~12.5KB JS / ~1.8KB CSS.** Ceiling is ~13KB JS; flag anything that moves it materially. (The old "~10KB budget" sat *below* actual for several releases, so it could never flag anything — keep this figure current when it moves, or it rots the same way. The marketing site quotes it too: `waveform-site/src/data/packages.ts`.)
